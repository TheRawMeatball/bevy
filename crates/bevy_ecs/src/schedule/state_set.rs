...
impl<T: Clone + Resource> StateSetBuilder<T> {
    ...
    pub fn finalize(self) -> SystemSet {
        ...
        let mut root_set = SystemSet::default().with_run_criteria(root_run_criteria::<T>.system());

        for (val, set) in self.on_enter.into_iter() {
            root_set.add_child(set.with_run_criteria(SetState::<T>::on_enter(val)));
        }
        for (val, set) in self.on_update.into_iter() {
            root_set.add_child(set.with_run_criteria(SetState::<T>::on_update(val)));
        }
        for (val, set) in self.on_exit.into_iter() {
            root_set.add_child(set.with_run_criteria(SetState::<T>::on_exit(val)));
        }
        for (val, set) in self.on_pause.into_iter() {
            root_set.add_child(set.with_run_criteria(SetState::<T>::on_pause(val)));
        }
