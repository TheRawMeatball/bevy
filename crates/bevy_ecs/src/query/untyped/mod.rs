...
use super::{OptionState, UntypedFetchState, UntypedReadState, UntypedWriteState};

pub struct UntypedQueryState {
    world_id: WorldId,
    ...
    pub(crate) fetch_state: Vec<UntypedState>,
    pub(crate) filter_state: Vec<UntypedState>,
}
...
pub enum QueryDescriptor {
    ...
    Mutable(ComponentId),
    OptImmutable(ComponentId),
    OptMutable(ComponentId),
}

pub enum QueryFilterDescriptor {
    With(ComponentId),
    Without(ComponentId),
    Changed(ComponentId),
    Added(ComponentId),
...
enum UntypedState {
    Entity,
    Immutable(UntypedReadState),
    Mutable(UntypedWriteState),
    OptImmutable(OptionState<UntypedReadState>),
    OptMutable(OptionState<UntypedWriteState>),
}

impl UntypedQueryState {
    pub fn new(
        world: &mut World,
        fetch: Vec<QueryDescriptor>,
        filter: Vec<QueryFilterDescriptor>,
    ) -> Self {
        let mut component_access = FilteredAccess::default();
        let fetch_state: Vec<_> = fetch
            .into_iter()
            .map(|v| match v {
                QueryDescriptor::Entity => UntypedState::Entity,
                QueryDescriptor::Immutable(component_id) => {
                    let state = UntypedReadState::new(component_id, world).unwrap();
                    state.update_component_access(&mut component_access);
                    UntypedState::Immutable(state)
                }
                QueryDescriptor::Mutable(component_id) => {
                    let state = UntypedWriteState::new(component_id, world).unwrap();
                    state.update_component_access(&mut component_access);
                    UntypedState::Mutable(state)
                }
                QueryDescriptor::OptImmutable(component_id) => {
                    let state =
                        OptionState::new(UntypedReadState::new(component_id, world).unwrap());
                    state.update_component_access(&mut component_access);
                    UntypedState::OptImmutable(state)
                }
                QueryDescriptor::OptMutable(component_id) => {
                    let state =
                        OptionState::new(UntypedWriteState::new(component_id, world).unwrap());
                    state.update_component_access(&mut component_access);
                    UntypedState::OptMutable(state)
                }
            })
            .collect();
        ...
        let filter_state: Vec<_> = filter
            .into_iter()
            .map(|v| match v {
                QueryFilterDescriptor::With(_) => todo!(),
                QueryFilterDescriptor::Without(_) => todo!(),
                QueryFilterDescriptor::Changed(_) => todo!(),
                QueryFilterDescriptor::Added(_) => todo!(),
            })
            .collect();
    ...
    #[inline]
    pub unsafe fn get_unchecked<'w, 'b>(
        &mut self,
        world: &'w World,
        entity: Entity,
        return_buffer: &'b mut [*mut u8],
    ...
    /// This does not check for mutable query correctness. To be safe, make sure mutable queries
    /// have unique access to the components they query.
    pub unsafe fn get_unchecked_manual<'w, 'b>(
        &self,
        world: &'w World,
        entity: Entity,
